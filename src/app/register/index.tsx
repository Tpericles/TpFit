...
export default function Register(){
  ...
  const handleLogin = async ({ email, senha, nome }: any) => {
    await new Promise(resolve => setTimeout(resolve, 1000));
    try {
      await AsyncStorage.setItem('@deliverymanager:user', nome);
    ...
    <Image source={require("@/assets/logo.png")} className=" h-14 w-32 self-center"/>
      <Formik
        ...
        {({errors, touched, handleBlur, handleChange, handleSubmit, isSubmitting}) => (
          <>
            <Text className="text-2xl font-bold mt-5 self-center">Criar conta</Text>
            ...
            <View className=" self-center flex-row mt-4">
              ...
              <TouchableOpacity onPress={() => router.push('/')}>
                <Text className="font-bold text-primary">Fazer login</Text>
              </TouchableOpacity>
            </View>
